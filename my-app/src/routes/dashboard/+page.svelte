...
//
    ...
    async function fetchSupa() {
        try {
            const { data, error } = await supabase.from('youtube_stats').select('*');
            if (error) {
 ...
 function link_regulator() {
    ...
    if (match) {
        ...
        async function fetchData() {
                try {
                    const response = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=id&part=snippet&part=statistics&id=${youtube_id}&key=AIzaSyDai-aOB7z7gZo9Os9CbfAkPkhInBhPzNE`);
                    ...
                    .from('youtube_stats')
                    .insert([
                        { video_id: youtube_id, title: video_title, views: video_views, likes: video_likes, comments: video_comments },
                    ])
                    .select()
...
<section>
    <div class="flex flex-col">
        ...
        <form on:submit={link_regulator} class="w-full flex justify-between">
            <input class=" w-full mr-2 text-primary pl-4" id="input_yt" type="text" bind:value={youtube_link}>
            <Button type="submit" class="w-1/6 bg-secondary text-secondary-foreground hover:underline hover:bg-secondary">Ottieni report</Button>
        </form>
    </div>
    <div class="mt-8">
        <Table.Root>
            <Table.Header>
            <Table.Row>
                <Table.Head class="w-[100px] text-bold">ID</Table.Head>
